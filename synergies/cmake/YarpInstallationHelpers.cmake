...
macro(YARP_CONFIGURE_EXTERNAL_INSTALLATION _name)
    if (YARP_INSTALL_PREFIX) #if YARP is installed
        ...
        if (same_path)
            ...
            set(BUFFER "${BUFFER}path \"${CMAKE_INSTALL_PREFIX}/${CMAKE_INSTALL_DATAROOTDIR}/${_name}\"")
            file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/path.d/${_name}.ini "${BUFFER}") #Create _name.ini file inside build directory
            install(FILES ${CMAKE_CURRENT_BINARY_DIR}/path.d/${_name}.ini DESTINATION ${YARP_INSTALL_PREFIX}/${CMAKE_INSTALL_DATAROOTDIR}/yarp/config/path.d) #Install the file into yarp config dir
            message(STATUS "Setting up installation of ${_name}.ini to ${YARP_INSTALL_PREFIX}/${CMAKE_INSTALL_DATAROOTDIR}/yarp/config/path.d folder.")
        else()
            if(WIN32)
                set(PATH_SEPARATOR ";")
            else()
            ...
            if("$ENV{YARP_DATA_DIRS}" STREQUAL "")
                message(WARNING "Installation prefix is different from YARP's : no file will we be installed into path.d folder, you need to set YARP_DATA_DIRS environment variable to ${YARP_INSTALL_PREFIX}/${CMAKE_INSTALL_DATAROOTDIR}/yarp${PATH_SEPARATOR}${CMAKE_INSTALL_PREFIX}/${CMAKE_INSTALL_DATAROOTDIR}/${_name}")
            else()
                message(WARNING "Installation prefix is different from YARP's : no file will we be installed into path.d folder, you need to set YARP_DATA_DIRS environment variable to $ENV{YARP_DATA_DIRS}${PATH_SEPARATOR}${CMAKE_INSTALL_PREFIX}/${CMAKE_INSTALL_DATAROOTDIR}/${_name}")
            endif()
        endif()
    ...
    set(${_NAME}_DATA_INSTALL_DIR ${CMAKE_INSTALL_DATAROOTDIR}/${_name} CACHE INTERNAL "general data installation directory for ${_name} (relative to build/installation dir")
